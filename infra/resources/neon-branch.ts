...
export class NeonBranchProvider implements pulumi.dynamic.ResourceProvider {
  ...
  private async getProjectByName(projectName: string) {
    ...
    return listProjectsResponse.projects.find(
  ...
  private async getBranch(projectId: string, branchId: string) {
    try {
      const branchResponse = await this.neonClient.getProjectBranch(
        projectId,
        branchId
      );
  ...
  private async createBranch(
    ...
    const createBranchResponse = await this.neonClient.createProjectBranch(
      projectId,
      {
        branch: {
          name: branchName,
  ...
  private async updateBranch(
    ...
    for (const existingEndpoint of existingEndpoints) {
      if (existingEndpoint.type === "read_write") {
        ...
        if (readWriteOption) {
          endpointOptions = endpointOptions.filter(
            (_, index) => index !== readWriteOptionIndex
          );
          await this.neonClient.updateProjectEndpoint(
            projectId,
            existingEndpoint.id,
            {
              endpoint: {
                provisioner:
                  readWriteOption.provisioner || ("k8s-neonvm" as Provisioner),
                autoscaling_limit_min_cu:
                  readWriteOption.autoscaling_limit_min_cu || 0.25,
                autoscaling_limit_max_cu:
                  readWriteOption.autoscaling_limit_max_cu || 1,
                suspend_timeout_seconds:
                  readWriteOption.suspend_timeout_seconds || 0,
              },
            }
          );
        }
        ...
        if (readOnlyOption) {
          endpointOptions = endpointOptions.filter(
            (_, index) => index !== readOnlyOptionIndex
          );
          await this.neonClient.updateProjectEndpoint(
            projectId,
            existingEndpoint.id,
            {
              endpoint: {
                provisioner:
                  readOnlyOption.provisioner || ("k8s-neonvm" as Provisioner),
                autoscaling_limit_min_cu:
                  readOnlyOption.autoscaling_limit_min_cu || 0.25,
                autoscaling_limit_max_cu:
                  readOnlyOption.autoscaling_limit_max_cu || 1,
                suspend_timeout_seconds:
                  readOnlyOption.suspend_timeout_seconds || 0,
              },
            }
          );
        } else {
          await this.neonClient.deleteProjectEndpoint(
            projectId,
            existingEndpoint.id
          );
        }
    ...
    for (const endpointOption of endpointOptions) {
      await this.neonClient.createProjectEndpoint(projectId, {
        endpoint: {
          ...
          provisioner:
            endpointOption.provisioner || ("k8s-neonvm" as Provisioner),
          autoscaling_limit_min_cu:
            endpointOption.autoscaling_limit_min_cu || 0.25,
          autoscaling_limit_max_cu:
            endpointOption.autoscaling_limit_max_cu || 1,
  ...
  private async getAllNeonConnectionUrls(
    ...
    const rolesResponse = await this.neonClient.listProjectBranchRoles(
      projectId,
      branchId
    );
    ...
    if (!rolePassword) {
      const rolePasswordResponse =
        await this.neonClient.getProjectBranchRolePassword(
          projectId,
          branchId,
          role.name
        );
    ...
    const databasesResponse = await this.neonClient.listProjectBranchDatabases(
      projectId,
      branchId
    );
    return this.formConnectionUrls(
  ...
  private getDatabasesFromConnectionUrls(connectionUrls: NeonConnectionUrl[]) {
    const readWriteUrl = connectionUrls.find(
      (url) => url.type === "read_write"
    )?.url;
    if (!readWriteUrl) {
      throw new Error("No readwrite database found");
    }
    const readOnlyUrl =
      connectionUrls.find((url) => url.type === "read_only")?.url ||
      readWriteUrl;
  ...
  async create({
    ...
    const connectionUrls = await this.getAllNeonConnectionUrls(
      project.id,
      branch!.id,
      roleName
    );
...
export class NeonBranch extends pulumi.dynamic.Resource {
  ...
  constructor(
    name: string,
    args: NeonBranchInputs,
    opts?: pulumi.CustomResourceOptions
  ) {
    super(
      new NeonBranchProvider(process.env.NEON_API_KEY!),
      name,
      { ...args, readOnlyUrl: undefined, readWriteUrl: undefined },
      opts,
      "theaistudybible",
      "NeonBranch"
    );
