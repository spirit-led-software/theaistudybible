...
export async function createUserGeneratedImage(
  data: CreateUserGeneratedImageData
) {
  return (
    await readWriteDatabase.insert(userGeneratedImages).values(data).returning()
  )[0];
}

export async function updateUserGeneratedImage(
  id: string,
  data: UpdateUserGeneratedImageData
) {
  return (
