<script lang="ts">
  ...
  import { updateDataSource } from '@revelationsai/client/services/admin/data-source';
...
<dialog bind:this={editDialog} class="modal">
  <form on:submit|preventDefault={handleSubmitEdit} class="modal-box space-y-2 p-10">
    ...
    />
    {#if dataSource.type !== 'FILE'}
      <select class="select w-full" name="syncSchedule">
    ...
    <form method="dialog" class="flex justify-end space-x-2">
      <button class="btn btn-sm btn-circle btn-ghost absolute right-2 top-2" disabled={isLoading}>
        x
      </button>
    </form>
  </form>
