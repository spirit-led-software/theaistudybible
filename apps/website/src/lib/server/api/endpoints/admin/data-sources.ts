    ...
    return c.json(
      {
        ...
        nextCursor:
          foundDataSources.length < limit ? undefined : cursor + limit,
    ...
    async (c) => {
      ...
      const [sourceDocumentIds, sourceDocumentCount] = await Promise.all([
        db.query.dataSourcesToSourceDocuments
          .findMany({
            where: eq(
              dataSourcesToSourceDocuments.dataSourceId,
              c.var.dataSource.id
            ),
            limit,
            offset: cursor,
        ...
        db
          ...
          .where(
            eq(dataSourcesToSourceDocuments.dataSourceId, c.var.dataSource.id)
          )
      ...
      return c.json(
        {
          ...
          nextCursor:
            sourceDocumentIds.length < limit ? undefined : cursor + limit,
      ...
      return c.json(
        {
          ...
          nextCursor:
            foundIndexOperations.length < limit ? undefined : cursor + limit,
