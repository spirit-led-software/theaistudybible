...
export const app = new Hono<{
  ...
  .get(
    "/",
    zValidator("query", PaginationSchema(userGeneratedImages)),
    async (c) => {
      const { cursor, limit, filter, sort } = c.req.valid("query");

      let where: SQL<unknown> | undefined = eq(
        userGeneratedImages.userId,
        c.var.clerkAuth!.userId!
      );
      if (filter) {
  ...
  .get("/:id/source-documents", async (c) => {
    const sourceDocumentRelations =
      await db.query.userGeneratedImagesToSourceDocuments.findMany({
        where: eq(userGeneratedImages.id, c.var.image.id),
      });
