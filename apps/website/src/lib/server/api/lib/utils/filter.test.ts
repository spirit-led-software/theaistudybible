...
describe("Filter string test suite", () => {
  test("Test filter string 1", () => {
    const sql = parseFilterString(
      chats,
      'id = "1234" and (name = "123" or customName = false)'
    );
    expect(sql).toEqual(
      and(
        eq(chats.id, "1234"),
        or(eq(chats.name, "123"), eq(chats.customName, false))
      )
    );
  });
  ...
  test("Test filter string 2", () => {
    ...
    expect(sql).toEqual(
      or(
        and(eq(chats.id, "1234"), eq(chats.name, "123")),
        eq(chats.customName, false)
      )
    );
  });
