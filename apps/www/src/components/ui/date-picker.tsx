...
const DatePickerRangeText = (props: DatePickerPrimitive.RangeTextProps) => {
  const [local, others] = splitProps(props, ['class']);
  return (
    <DatePickerPrimitive.RangeText class={cn('font-medium text-sm', local.class)} {...others} />
  );
};
...
const DatePickerTableHeader = (props: DatePickerPrimitive.TableHeaderProps) => {
  ...
  return (
    <DatePickerPrimitive.TableHeader
      class={cn('w-8 flex-1 font-normal text-[0.8rem] text-muted-foreground', local.class)}
      {...others}
    />
  );
};
