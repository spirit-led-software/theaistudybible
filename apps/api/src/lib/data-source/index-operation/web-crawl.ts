  ...
  try {
    ...
    if (indexableUrls.length > 0) {
      ...
      for (let i = 0; i < indexableUrls.length; i += sliceSize) {
        ...
        try {
          await sendUrlsToQueue({ name, urls: indexableUrlsSlice, indexOpId });
    ...
    try {
      for (const additionalSitemap of additionalSitemaps) {
        ...
        const res = await fetch(additionalSitemap, {
          method: 'GET'
        });
        if (!res.ok) {
          throw new Error(`Failed to fetch additional sitemap ${additionalSitemap}`);
        }
        urlCount += await navigateSitemap({
          sitemapXml: await res.text(),
          urlRegex,
    ...
    try {
      for (const gZippedSitemap of gZippedSitemaps) {
        ...
        const res = await fetch(gZippedSitemap, {
          method: 'GET'
        });
        if (!res.ok) {
          throw new Error(`Failed to fetch gzipped sitemap ${gZippedSitemap}`);
        }
        const gZippedSitmapData = await res.arrayBuffer();
...
async function sendUrlsToQueue({
  ...
  const sendMessageCommand = new SendMessageBatchCommand({
    QueueUrl: Resource.Queue.webpageIndexQueue.queueUrl,
...
export async function checkIfIndexOpIsCompletedAndUpdate(indexOp: IndexOperation) {
  try {
