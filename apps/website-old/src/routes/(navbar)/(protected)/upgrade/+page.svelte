<script lang="ts">
  import PricingCard from '$lib/components/PricingCard.svelte';
  import { hasPlus, isAdmin } from '@revelationsai/client/services/user';
  import type { PageData } from './$types';

  export let data: PageData;

  $: ({ user, productInfos } = data);
</script>

<svelte:head>
  <title>Upgrade Account</title>
  <meta name="description" content="Upgrade your account to access more queries" />
</svelte:head>

<div class="flex h-full w-full flex-col place-items-center space-y-4 overflow-y-scroll py-5">
  <div class="flex flex-col space-y-2 text-center text-xl">
    <h1>Remaining Queries:</h1>
    <h2 class="text-base">
      <span
        class={`${
          user.remainingQueries < 3
            ? 'text-red-500'
            : user.remainingQueries < 5
              ? 'text-yellow-500'
              : 'text-green-500'
        }`}
      >
        {hasPlus(user) || isAdmin(user) ? 'Unlimited' : user.remainingQueries}
      </span>{' '}
      of {hasPlus(user) || isAdmin(user) ? 'Unlimited' : user.maxQueries}
    </h2>
    <a
      href={`https://checkout.revelationsai.com/p/login/bIY5mO0MW95xgQ8288?prefilled_email=${encodeURIComponent(user.email)}`}
      target="_blank"
    >
      <button class="rounded-lg bg-slate-700 px-3 py-2 text-white hover:bg-slate-900">
        View Current Plan
      </button>
    </a>
  </div>
  <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
    <PricingCard
      title="RevelationsAI Standard"
      price="Free"
      features={[
        'GPT-3.5',
        'Send 5 messages every hour',
        'Generate 1 image every 8 hours (mobile only)',
        'Standard ads'
      ]}
      currentLevel={!hasPlus(user)}
    />
    {#each productInfos as productInfo}
      <PricingCard
        title={productInfo.product.name}
        price={productInfo.product.default_price}
        features={productInfo.product.features
          .filter((feature) => feature.name)
          .map((feature) => feature.name)}
        currentLevel={hasPlus(user)}
        purchaseLink={productInfo.paymentLink.url}
      />
    {/each}
  </div>
</div>
