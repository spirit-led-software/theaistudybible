...
interface EmailPasswordConfig {
  ...
  onForgotPasswordCallback: (token: string) => Promise<APIGatewayProxyStructuredResultV2>;
  onResetPassword: (email: string, password: string) => Promise<APIGatewayProxyStructuredResultV2>;
  onError: (error: unknown) => Promise<APIGatewayProxyStructuredResultV2>;
}

export const CredentialsAdapter = createAdapter((config: EmailPasswordConfig) => {
    ...
    if (step === 'register') {
      try {
        ...
        const registerCallback =
          'https://' + [useDomainName(), ...usePath().slice(0, -1), 'register-callback'].join('/');
        const claims: {
          email: string;
          password: string;
        } = JSON.parse(useBody() || '{}');
      ...
      try {
        ...
        const forgotPasswordCallback =
          'https://' +
          [useDomainName(), ...usePath().slice(0, -1), 'forgot-password-callback'].join('/');
